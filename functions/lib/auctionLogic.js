...
exports.applySelections = exports.detectConflicts = exports.assignHighestBidder = exports.sumRoomPrices = exports.initAuction = void 0;
/**
 ...
 */
const initAuction = (id, totalRent, roomNames, userNames) => {
...
exports.initAuction = initAuction;
/**
 * Calculates the sum of all room prices in a given list of rooms.
 */
const sumRoomPrices = (rooms) => {
    return Number(rooms.reduce((s, r) => s + r.price, 0).toFixed(2));
};
exports.sumRoomPrices = sumRoomPrices;
/**
 * Assigns the highest bidder to a specific room and recalculates the prices
 * for all remaining unassigned rooms.
 */
const assignHighestBidder = (auction, roomId, bids) => {
    // Find highest bid; if tie, return auction unchanged (caller should handle re-bid)
    if (bids.length === 0)
        return auction;
    let highest = bids[0];
    for (const b of bids) {
        if (b.amount > highest.amount)
            highest = b;
    }
    const rooms = auction.rooms.map(r => (Object.assign({}, r)));
    const users = auction.users.map(u => (Object.assign(Object.assign({}, u), { assignedRoomId: u.assignedRoomId })));
    ...
    return Object.assign(Object.assign({}, auction), { rooms, users });
};
exports.assignHighestBidder = assignHighestBidder;
/**
 * Analyzes user room selections to detect conflicts, where multiple users
 * have selected the same room.
 */
const detectConflicts = (auction, selections) => {
    var _a, _b, _c;
    ...
    for (const user of auction.users) {
        ...
        const sel = (_b = (_a = selections[user.id]) !== null && _a !== void 0 ? _a : user.assignedRoomId) !== null && _b !== void 0 ? _b : null;
        if (!sel)
            continue;
        roomToUsers[sel] = (_c = roomToUsers[sel]) !== null && _c !== void 0 ? _c : [];
 ...
 */
const applySelections = (auction, selections) => {
    var _a, _b;
    ...
    const rooms = auction.rooms.map(r => (Object.assign({}, r)));
    const users = auction.users.map(u => (Object.assign({}, u)));
    const chosen = {};
    for (const u of users) {
        // skip users already assigned
        if (u.assignedRoomId)
            continue;
        const sel = (_a = selections[u.id]) !== null && _a !== void 0 ? _a : null;
        if (!sel)
            continue;
        chosen[sel] = (_b = chosen[sel]) !== null && _b !== void 0 ? _b : [];
...
exports.applySelections = applySelections;
//# sourceMappingURL=auctionLogic.js.map